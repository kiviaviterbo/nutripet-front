...
export default function NutritionTable() {
  ...
  const [showFilters, setShowFilters] = useState(false); // ← NOVO
  ...
  // Carregar ração quando filtros mudam
  useEffect(() => {
    carregarRacoes();
          ...
          <main className="content">
            ...
            {loading ? (
              ...
              <div className="feed-grid">
                {feeds.length === 0 ? (
                  <p className="no-results">Nenhuma ração encontrada.</p>
                ) : (
                  feeds.map((feed) => (
                    <div
                      key={feed.id}
                    ...
                    >
                      ...
                      <h5>{feed.nome}</h5>
                      <p>{feed.marca}</p>
                      <small>{feed.tipo} • {feed.especie} • {feed.variacao}</small>
                    </div>
                  ))
                )}
              </div>
